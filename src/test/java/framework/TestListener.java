...
public class TestListener implements ITestListener {
    ...
    public void onTestSkipped(ITestResult iTestResult) {
        System.out.println(String.format("======================= SKIPPED TEST %s Duration: %ss =======================",
                iTestResult.getName()));
    }

    private long getExecutionTime(ITestResult iTestResult) {
        return TimeUnit.MILLISECONDS.toSeconds((iTestResult.getEndMillis() - iTestResult.getEndMillis()));
    }
...
//    }
    ...
    private static byte[] expectedResult(String resourceName) throws IOException {
        return Files.readAllBytes(Paths.get("src/test/resources", resourceName));
    }

    @Attachment(value = "Avtual result", fileExtension = ".txt")
    private static byte[] actualResult(String resourceName) throws IOException {
        return Files.readAllBytes(Paths.get("src/test/resources", resourceName));
    }
